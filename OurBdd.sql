...
-- (aligne equipements/tickets). PK (id) globale sur TS_GLPI_INDX.
...
-- glpi_tickets : partitionnee par site (entities_id) car la volumetrie
-- grossit avec l'activite et chaque ticket appartient a un site precis.
CREATE TABLE glpi_tickets (
  id NUMBER GENERATED BY DEFAULT AS IDENTITY,
  name VARCHAR2(255),
  content CLOB,
  date_issue TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- Automatisation de la date
  status NUMBER DEFAULT 1 NOT NULL,               -- 1 = Nouveau, par défaut
  entities_id NUMBER NOT NULL,                    -- NOT NULL pour partitionnement
  locations_id NUMBER DEFAULT NULL,
...
CREATE TABLE glpi_profiles_users (
  id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  users_id NUMBER NOT NULL,    -- Pas de NULL ici car c'est une table de liaison
  profiles_id NUMBER NOT NULL, -- Idem
  entities_id NUMBER NOT NULL,
...
-- ---- Tables d'actifs / lieux physique par site ----------------------------
-- Les lieux ont ete declares juste apres entities (liste ci-dessus), deja en
-- PARTITION LIST(entities_id) sur TS_GLPI_CERGY / TS_GLPI_PAU.
-- PARTITION BY LIST(entities_id) : 1 = p_cergy, 2 = p_pau (sans partition DEFAULT :
-- tout entities_id doit etre 1 ou 2 sinon CHECK echoue avant insertion physique).
-- Bénéfices :
...
-- glpi_equipments : table MERE (heritage simule). Contient entities_id donc
-- elle est partitionnee par site. C'est elle qui porte le gros volume.
...
-- glpi_computers et glpi_printers : tables FILLES de glpi_equipments.
-- Elles n'ont plus entities_id en propre, donc on utilise PARTITION BY
-- REFERENCE : les partitions des enfants suivent automatiquement celles
-- du parent grace a la FK fk_comp_inherit / fk_print_inherit.
-- Resultat : la ligne computer du PC-001 (entities_id=1 dans equipments)
-- atterrit aussi dans p_cergy cote computers, donc dans TS_GLPI_CERGY.
-- C'est ce que Oracle appelle "co-location" des donnees liees.
CREATE TABLE glpi_computers (
  ...
  CONSTRAINT fk_comp_inherit FOREIGN KEY (id) REFERENCES glpi_equipments(id) ON DELETE CASCADE
)
...
CREATE TABLE glpi_printers (
  ...
  CONSTRAINT fk_print_inherit FOREIGN KEY (id) REFERENCES glpi_equipments(id) ON DELETE CASCADE
)
PARTITION BY REFERENCE (fk_print_inherit);

-- glpi_ipaddresses : la nouvelle version ne porte plus entities_id (la
-- relation passe par networks_id -> glpi_networks.entities_id). On la
-- laisse non partitionnee, dans TS_GLPI_REF (petite table, lookup
-- frequent depuis les jointures).
...
-- Sur les tables PARTITIONNEES par entities_id (locations, equipments, computers, printers, tickets)
...
-- Index Uniques --------------------------------------------------------------
-- Note : uk_ent_id_name (ancienne version) n'a plus de sens car la nouvelle
-- structure de glpi_entities ne contient plus d'auto-reference (entities_id).
CREATE UNIQUE INDEX uk_ent_name           ON glpi_entities      (name) TABLESPACE TS_GLPI_INDX;
CREATE UNIQUE INDEX uk_loc_ent_loc_name ON glpi_locations (entities_id, locations_id, name) LOCAL TABLESPACE TS_GLPI_INDX;
CREATE UNIQUE INDEX uk_profrights_prof_name ON glpi_profilerights (profiles_id, name) TABLESPACE TS_GLPI_INDX;

-- Index glpi_tickets (table partitionnee -> index LOCAL) ---------------------
-- LOCAL : 1 sous-index par partition. Une recherche par status sur Cergy
-- ne lit que le sous-index de la partition p_cergy (partition pruning).
...
CREATE INDEX idx_loc_loc_id ON glpi_locations (locations_id) LOCAL TABLESPACE TS_GLPI_INDX;
...
-- Index glpi_equipments (Table Mere, partitionnee -> LOCAL) ------------------
...
-- Index glpi_computers (Table Enfant, partitionnee BY REFERENCE -> LOCAL) ----
...
-- Index glpi_printers (Table Enfant, partitionnee BY REFERENCE -> LOCAL) -----
...
-- CREATE TABLE (necessaire pour PARTITION BY REFERENCE), donc rien a
-- ajouter ici.
